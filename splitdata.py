...
for i in range(1,subject+1):
    ...
    print("\nP{}.csv".format(i+4))
    line_count = 0
    # use the raw csv files to populate frame csv files for each subject
    rootdir = Path('Data/ALL')
    file_list = [f for f in rootdir.glob('**/*') if f.is_file()]
    for cur in file_list:
        with open(cur, 'a') as source:
            name_writer = csv.writer(source, delimiter=',')
            name_writer.writerow(['P{}.csv'.format(i+4),'']) # output start and end frame to its specific procedure csv in ~/ALL
        ...
        with open('framecsv/P{}frame.csv'.format(i+4), 'w') as output:
            ...
            for row in csv_reader:
                ...
                if line_count == 0:
                    ...
                    with open('Data/ALL/{}.csv'.format(row[1]), 'a') as procedure_file:
